...
import { extractSubscriptionExpiryDateFromIdToken } from '@/utils/jwt'
...
// Extract subscription expiry from credentials or id_token as fallback
const effectiveSubscriptionExpiresAt = computed(() => {
  // Priority 1: Use subscription_expires_at from credentials if available
  if (props.subscriptionExpiresAt) {
    return props.subscriptionExpiresAt
  }

  // Priority 2: Try to extract from id_token in credentials
  if (props.credentials && typeof props.credentials === 'object') {
    const idToken = props.credentials['id_token'] as string | undefined
    if (idToken && typeof idToken === 'string') {
      const expiryFromToken = extractSubscriptionExpiryDateFromIdToken(idToken)
      if (expiryFromToken) {
...
const expiresLabel = computed(() => {
  const expiresAt = effectiveSubscriptionExpiresAt.value
  if (!expiresAt || !props.planType) return ''
  if (props.planType.toLowerCase() === 'free') return ''
  try {
    const d = new Date(expiresAt)
    if (isNaN(d.getTime())) return ''
    // Use UTC methods to ensure consistent date formatting regardless of timezone
