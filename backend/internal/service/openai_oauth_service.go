...
// It does NOT perform OAuth refresh grant. Only reads metadata and updates specified fields.
// Preserves all existing token fields (access_token, refresh_token, id_token, client_id, expires_at).
func (s *OpenAIOAuthService) SyncAccountMetadata(ctx context.Context, account *Account) (*OpenAITokenInfo, error) {
	if account.Platform != PlatformOpenAI {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_INVALID_ACCOUNT", "account is not an OpenAI account")
	}
	if account.Type != AccountTypeOAuth {
	...
	accessToken := account.GetCredential("access_token")
	if accessToken == "" {
		return nil, infraerrors.New(http.StatusBadRequest, "OPENAI_OAUTH_NO_ACCESS_TOKEN", "no access token available for account")
	}
	...
	// Build token info from existing credentials
	tokenInfo := &OpenAITokenInfo{
		AccessToken:      accessToken,
		RefreshToken:     account.GetCredential("refresh_token"),
		IDToken:          account.GetCredential("id_token"),
	...
	// Extract organization ID from access_token JWT if not available
	orgID := tokenInfo.OrganizationID
	if orgID == "" {
	...
	// Fetch account info from ChatGPT backend API (readonly)
	...
	// Try to extract subscription expiry from id_token if not available from API
	if tokenInfo.SubscriptionExpiresAt == "" && tokenInfo.IDToken != "" {
		if claims, err := openai.DecodeIDToken(tokenInfo.IDToken); err == nil && claims.OpenAIAuth != nil {
			// Check if there's subscription expiry in the claims
			// This is a fallback for cases where the API doesn't return it
			if claims.Exp > 0 {
				// Note: This is not subscription expiry, this is id_token expiry
				// We keep this for future use if OpenAI adds subscription expiry to id_token claims
			}
		}
	}
...
// BuildSyncAccountCredentials builds credentials map from token info for sync operation.
// It only includes fields that should be updated, preserving all other existing credentials.
func (s *OpenAIOAuthService) BuildSyncAccountCredentials(tokenInfo *OpenAITokenInfo) map[string]any {
	creds := map[string]any{}

	// Only include fields that were actually synced
	if tokenInfo.Email != "" {
		creds["email"] = tokenInfo.Email
	}
	if tokenInfo.ChatGPTAccountID != "" {
